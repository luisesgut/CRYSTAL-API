...
namespace WebOs.Controllers
{
    public class ReporteFechaYMaquinaController : ApiController
    {
        ...
        [HttpGet]
        [Route("api/fechaYmaquina")]
        public IHttpActionResult GenerarPorFechaYMaquina(string fecha, string maquina)
        {
            try
            {
                // Validar parámetros
                if (string.IsNullOrEmpty(fecha) || string.IsNullOrEmpty(maquina))
                    return BadRequest("❌ Los parámetros 'fecha' y 'maquina' son obligatorios.");
                ...
                // Crear nombre único del archivo
                string timestamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
                string fileName = $"Máquina_{fecha}_{maquina}_{timestamp}.pdf";
